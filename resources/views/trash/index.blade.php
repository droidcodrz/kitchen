                ...
                @foreach($filters as $value => $label)
                    <a href="{{ route('trash.index', ['type' => $value]) }}"
                       class="whitespace-nowrap px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 border
                        ...
                        {{ $label }}
                        @if($value === 'all')
                            ({{ $trashedProjects->count() + $trashedProducts->count() + $trashedCategories->count() + $trashedFolders->count() }})
                        @elseif($value === 'projects')
                            ({{ $trashedProjects->count() }})
                        @elseif($value === 'products')
                            ({{ $trashedProducts->count() }})
                        @elseif($value === 'categories')
                            ({{ $trashedCategories->count() }})
                        @elseif($value === 'folders')
                            ({{ $trashedFolders->count() }})
                        @endif
                    </a>
                @endforeach
            </div>
                                ...
                                @foreach($trashedProjects as $project)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <form method="POST" action="{{ route('trash.restore', ['type' => 'project', 'id' => $project->id]) }}" class="inline">
                                                @csrf
                            ...
                            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                                @foreach($trashedProducts as $product)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <form method="POST" action="{{ route('trash.restore', ['type' => 'product', 'id' => $product->id]) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 mr-3">
                                                    Restore
                                                </button>
                            ...
                            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                                @foreach($trashedCategories as $category)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <form method="POST" action="{{ route('trash.restore', ['type' => 'category', 'id' => $category->id]) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 mr-3">
                                                    Restore
                                                </button>
                            ...
                            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                                @foreach($trashedFolders as $folder)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <form method="POST" action="{{ route('trash.restore', ['type' => 'folder', 'id' => $folder->id]) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 mr-3">
                                                    Restore
                                                </button>
