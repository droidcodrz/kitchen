...
@foreach ($navItems as $item)
    <a href="{{ route($item['route']) }}"
       ...
       @if(!$item['active']) title="{{ $item['label'] }}" @endif>
        <svg class="w-5 h-5 flex-shrink-0 {{ $item['active'] ? 'text-white dark:text-white' : 'text-gray-500 dark:text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            {!! $item['icon'] !!}
        </svg>
        <span x-show="sidebarOpen" x-cloak x-transition class="ml-3 whitespace-nowrap">{{ $item['label'] }}</span>
    </a>
@endforeach
