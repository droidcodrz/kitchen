<x-app-layout>
    ...
    <div class="bg-white dark:bg-gray-900 shadow-sm rounded-lg">
        <form method="POST" action="{{ route('projects.update', $project) }}" enctype="multipart/form-data" class="p-6 space-y-8">
                                ...
                                <select :name="'inventory_items[' + index + '][inventory_item_id]'" x-model="item.inventory_item_id" class="block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    @foreach($inventoryItems ?? [] as $inventoryItem)
                                        <option value="{{ $inventoryItem->id }}"
                                            :disabled="items.some((row, i) => i !== index && row.inventory_item_id == '{{ $inventoryItem->id }}')">{{ $inventoryItem->name }} ({{ trim('Inventory item' . ($inventoryItem->sku ? ' - ' . $inventoryItem->sku : '')) }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-32">
            ...
            <div x-data="{
                proposal: @js(old('proposal_signed_date', $project->proposal_signed_date?->format('Y-m-d'))),
                delivery: @js(old('delivery_date', $project->delivery_date?->format('Y-m-d')))
            }">
            ...
            <div x-data="attachmentUpload()">
                ...
                <template x-if="acceptedNames.length > 0">
                    <ul class="mt-3 space-y-1">
                        <template x-for="name in acceptedNames" :key="name">
